...
LINKC = '#1F4ED8'  # subtle blue so links read as clickable
...
name_s = ParagraphStyle('name', fontName='Helvetica-Bold', fontSize=21, leading=23, textColor=INK, spaceAfter=1)
title_s = ParagraphStyle('title', fontName='Helvetica', fontSize=11.5, leading=15, textColor=ACCENT, spaceAfter=5)
contact_s = ParagraphStyle('contact', fontName='Helvetica', fontSize=8.8, leading=12, textColor=GRAY)
summary_s = ParagraphStyle('summary', fontName='Helvetica', fontSize=9.6, leading=14, textColor=INK)
sec_s = ParagraphStyle('sec', fontName='Helvetica-Bold', fontSize=10, leading=12, textColor=INK, spaceBefore=11)
role_s = ParagraphStyle('role', fontName='Helvetica-Bold', fontSize=10.3, leading=13, textColor=INK, spaceBefore=6)
meta_s = ParagraphStyle('meta', fontName='Helvetica-Oblique', fontSize=8.8, leading=12, textColor=GRAY, spaceAfter=2)
bullet_s = ParagraphStyle('bullet', fontName='Helvetica', fontSize=9.3, leading=12.8, textColor=INK)
skill_s = ParagraphStyle('skill', fontName='Helvetica', fontSize=9.3, leading=13.6, textColor=INK)
...
def role(title, company, dates, place, items):
    story.append(Paragraph('%s &nbsp;&middot;&nbsp; %s' % (title, company), role_s))
    story.append(Paragraph('%s &nbsp;|&nbsp; %s' % (dates, place), meta_s))
    bullets(items)
...
story.append(Paragraph('Senior Product Designer', title_s))
...
story.append(Paragraph(
    ...
    'and consumer domains. I own the full arc, from discovery and research through UX, UI, prototyping and developer '
    'handoff, and I optimise for outcomes: products that ship, get used and move the metrics. Comfortable with '
    'ambiguity, data-heavy problems and design systems at scale, working closely with PMs, engineers and stakeholders.',
    summary_s))

# ---- experience ----
section('Experience')
role('Senior Product Designer', 'Exult Global', 'Feb 2024 - Present', 'India', [
    'Lead product design across enterprise and analytics products for clients including Wheels Up, HIG Capital, '
    'Dexcom and CyberProof.',
    'For Wheels Up (USA), partnered directly with the VP of Business Intelligence to consolidate 50+ internal '
    'dashboards into a scalable Figma design system, cutting report-generation time by 25% across operations, '
    'sales and finance.',
    'Own discovery through delivery: component-based dashboards and decision tools, validated with stakeholders '
    'and shipped with engineers through iterative design sprints and a documented design system.',
])
role('Product Designer', 'JIC IT Solutions', 'Jul 2023 - Feb 2024', 'India', [
...
role('Product Designer', 'WeCreate (Mintyfusion)', 'Jul 2021 - Apr 2023', 'Remote', [
...
role('Design Consultant', 'Freelance', 'Aug 2020 - Jul 2021', 'Remote', [
...
for lbl, items in [
    ...
    ('Craft', 'End-to-End UX/UI, Interaction Design, Prototyping, Information Architecture, Responsive Design'),
...
# ---- education ----
section('Education')
story.append(Paragraph('<b>B.Tech, Computer Science Engineering</b>', role_s))
story.append(Paragraph('MES College of Engineering, India &nbsp;|&nbsp; 2021', meta_s))
